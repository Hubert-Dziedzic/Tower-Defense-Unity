...
public class EnemyMovment : MonoBehaviour
{
    ...
    {
        indexOfPath = 0;
        nextPoint = LevelManager.main.pathPoints[indexOfPath];
    ...
    private void Update()
    {
        if (Vector2.Distance(nextPoint.position, _cachedTransform.position) <= 0.1f)
        {
            ...
            if (indexOfPath == LevelManager.main.pathPoints.Length)
            {
    ...
    private void FixedUpdate()
    {
        ...
        rb.velocity = direction * speed; // move enemy to next point
    }
